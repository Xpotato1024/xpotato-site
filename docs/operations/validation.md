...
ADR-0031 and `contracts/legacy-build-ranked-prefix-boundary-tie-amendment.md` are a **proposed, not-yet-accepted** bounded exception for exactly two frozen ranked finite-prefix call sites: Home latest Blog and Related Blog。It does not affect the current comparator before separate acceptance。If a fresh clean-room design audit passes and the operator explicitly accepts ADR-0031, a later implementation may admit membership differences only by the `ranked-prefix-boundary-tie-v1` proof: reconstruct the exact candidate universe from the frozen Git source, prove the strict prefix and cutoff tie group/cardinality, prohibit lower-ranked/out-of-universe selections, preserve unequal-key ordering, and validate selected-only cards against the complete frozen `ArticleCard compact=true` source+template representation including static classes/ARIA/data-* markup。All other membership differences remain failures。
