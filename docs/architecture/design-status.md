...
- vNext production deploy artifact determinism (ADR-0032): **PROPOSED 2026-09-24 / 未Accepted**。Astro 7.2.7のcheckout-path-dependent UIDをfinal distのbounded canonicalizationで除き、UTF-8 bytewise sorted manifestとbinary-framed tree SHA256を導入するreview candidate。ADR-0030のlegacy equivalenceを拡張しない。operator acceptanceと別途production deploy authorizationまではmain merge・provider mutation・JIT token・deployを行わない。
